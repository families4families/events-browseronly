...
<script type="x-tmpl-mustache" id="ConfirmationEmailDisplayCardTemplate">
    <style>
        ...
        {{#ClientOKSponsorReachOut}}
        <div class="family-contact-block">
        <table>
            <tr>
                <td>
                    <div class="family-contact-methods">
                        <h4 style="margin: 0px 0px 0px 0px;">Contact Methods</h4>
                ...
                <td>
                    <div style="margin-left: 50px;" class="family-contact-times">
                        <h4 style="margin: 0px 0px 0px 0px;">Contact Times</h4>
                        <table style="vertical-align:top;">
                            <thead></thead>
                            <tbody>
                                {{#ContactDayTime}}
                                <tr>
                                    <td>{{DaySpan}}</td>
                                    <td style="padding-left: 15px">{{TimeSpan}}</td>
                                </tr>
                                {{/ContactDayTime}}
                            </tbody>
                        </table>
        ...
        <div class="family-members">
            <table>
                ...
                <tr>
                    ...
                    <th>Pants</th>
                    <th>Shoes</th>
                    <th>Most Needed Items</th>
                    <th>Favorite Color or Character</th>
                    <th>Santa Wish List</th>
                    <th>Comments</th>
                </tr>
                </thead>
                <tbody>
                {{#FamilyMembers}}
                    <tr>
                        ...
                        <td>{{FMPantSize}}</td>
                        <td>{{FMShoeSize}}</td>
                        <td>{{FMMostNeededItems}}</td>
                        <td>{{FMFavoriteCustomization}}</td>
                        <td>{{FMSantaWishList}}</td>
                        <td>{{FMComments}}</td>
                    </tr>
                {{/FamilyMembers}}
                </tbody>
...
<script type="text/javascript">
    ...
    // family results
    const resultsPlaceholderSelector = 'div > div.sc-6991af84-0.jRJRAl.tally-text:contains("client family search results")';
    ...
    function applySearchCriteria(clientDataRows, criteria) {
        ...
        for (let key in criteria) {
            ...
            if (searchValue.includes("__gte")) {
                ...
                filters[key] = {
                    "filter": function(value, filterVal) { return value.match(filterVal); },
    ...
    function convertRowToObject(row) {
        ...
        const repeatingBlocksConfig = {
            "FM": {
                "columns": ["FMName", "FMGender", "FMAge", "FMSizingType", "FMShirtSize", "FMPantSize", "FMShoeSize", "FMMostNeededItems", "FMFavoriteCustomization", "FMSantaWishList", "FMComments"],
                "required": ["FMName"],
                "maxRepetitions": 10,
        ...
        /****
         * HOLIDAY 2024 specific processing
         */
        let phone = tree["ClientPhoneNumber"];
        if (phone.length === 11) {
            phone = phone.substring(1);
            phone = `(${phone.substring(0,3)}) ${phone.substring(3,6)}-${phone.substring(6)}`
        }
        tree["ClientPhoneNumber"] = phone;

        if (tree["ClientOKSponsorReachOut"] === "No") {
            tree["ClientOKSponsorReachOut"] = false;
        }


        // custom conversion for holiday contact method / times
        let contactMethods = row["ClientContactMethods"];
        contactMethods = contactMethods.split(",");
        contactMethods.forEach((cm) => {
            const method = cm.replaceAll(" Me", "").trim();
            tree[`Allows${method}`] = true;
        });


        const daySpans = ["Mon - Fri", "Sat - Sun"];
        const timeSpans = ["9am - 12pm", "12pm - 3pm", "3pm - 6pm", "6pm - 9pm"];
        const contactSpans = {};

        timeSpans.forEach((ts) => {
            daySpans.forEach((ds) => {
                if (row[ts] && row[ts].includes(ds)) {
                    if (!contactSpans[ds]) {
                        contactSpans[ds] = [];
                        contactSpans[ds].push(ts);
                    }
                    else if (contactSpans[ds][contactSpans[ds].length - 1].endsWith(ts.split(" - ")[0]) ) {
                        // combine adjacent time slots, i.e. 9am - 12pm, 12pm - 3pm --> 9am - 3pm
                        contactSpans[ds][contactSpans[ds].length - 1] =
                            contactSpans[ds][contactSpans[ds].length - 1].split(" - ")[0]
                            + " - "
                            + ts.split(" - ")[1];
                    } else {
                        contactSpans[ds].push(ts);
        ...
        const contactDayTimes = [];
        for (let key in contactSpans) {
            contactDayTimes.push({ "DaySpan": key, "TimeSpan": contactSpans[key].join(", ") });
        }
        tree["ContactDayTime"] = contactDayTimes;
        ...
        fetch(url, {
            ...
            .then((r) => {
                if (r.status === 200) {
                    ...
                    emailjs.send("service_y7z7za8", "template_gj7t0y6", templateParams, "ukws6XKmos438OOho")
                        .then(
                            (response) => {
